...
func ParseNode(k8sNode *v1.Node, source node.Source) *node.Node {
	...
	k8sNodeAddHostIP := func(annotation string) {
		if ciliumInternalIP, ok := k8sNode.Annotations[annotation]; !ok {
			scopedLog.Debug("Missing %s. Annotation required when IPSec Enabled", annotation)
		} else if ip := net.ParseIP(ciliumInternalIP); ip == nil {
			scopedLog.Debug("ParseIP %s error", ciliumInternalIP)
		} else {
			na := node.Address{
			...
			addrs = append(addrs, na)
			scopedLog.Debug("Add NodeCiliumInternalIP: %s", ip)
		}
	}
